...
use crate::{EffectId, EffectState, StoredValueId};
...
pub(crate) struct Root {
    ...
    pub(crate) root_scope: Cell<ScopeId>,
    ...
    pub(crate) signals: RefCell<SlotMap<SignalId, SignalState>>,
    ...
    pub(crate) stored_values: RefCell<SlotMap<StoredValueId, Rc<RefCell<dyn Any>>>>,
    ...
    /// it is called.
    pub(crate) rev_sorted_buf: RefCell<Vec<SignalId>>,
    /// A list of effects to be run after signal updates are over.
    pub(crate) effect_queue: RefCell<Vec<EffectId>>,
...
impl Root {
    ...
    #[cfg_attr(debug_assertions, track_caller)]
    pub(crate) fn propagate_updates(&self, start_node: SignalId) {
...
impl DependencyTracker {
    ...
    pub(crate) fn create_signal_dependency_links(self, root: &Root, dependent: SignalId) {
    ...
    pub(crate) fn create_effect_dependency_links(self, root: &Root, dependent: EffectId) {
        for signal in &self.dependencies {
            root.signals.borrow_mut()[*signal]
                .effect_dependents
                .push(dependent);
