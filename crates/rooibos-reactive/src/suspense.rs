...
impl SuspenseContext {
    ...
    pub fn new() -> Self {
        ...
        let pending_serializable_resources = create_rw_signal(Default::default());
    ...
    /// Notifies the suspense context that a new resource is now pending.
    pub(crate) fn increment_for_resource(&self, serializable: bool, resource: ResourceId) {
        let pending = self.pending;
    ...
    pub fn ready(&self) -> Memo<bool> {
        let pending = self.pending;
        create_memo(move |_| pending.try_with(|n| n.is_empty()).unwrap_or(false))
    }
}
