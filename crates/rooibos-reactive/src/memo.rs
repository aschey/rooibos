...
/// create a memo.
///
/// As with [`create_effect`](crate::create_effect), the argument to the memo function is the
/// previous value, i.e., the current value of the memo, which will be `None` for the initial
/// calculation.
///
/// ```
/// # use leptos_reactive::*;
...
/// 2. The function must also return whether the value has changed, as the first element of the
///    tuple.
///
...
///         if let Some(name) = old_name.filter(|old_name| old_name == &state.name) {
///             (name, false)
///         } else {
...
#[track_caller]
#[inline(always)]
pub fn create_owning_memo<T>(f: impl Fn(Option<T>) -> (T, bool) + 'static) -> Memo<T>
where
    T: PartialEq + 'static,
{
...
/// - [`.get()`](#impl-SignalGet<T>-for-Memo<T>) (or calling the signal as a function) clones the
...
/// - [`.with()`](#impl-SignalWith<T>-for-Memo<T>) allows you to reactively access the signal's
///   value without cloning by applying a callback function.
///   - [`.with_untracked()`](#impl-SignalWithUntracked<T>-for-Memo<T>) allows you to access the
///     signal's
///   value without reactively tracking it.
/// - [`.to_stream()`](#impl-SignalStream<T>-for-Memo<T>) converts the signal to an `async` stream
///   of values.
///
/// ## Examples
/// ```
...
impl<T> SignalWithUntracked for Memo<T> {
    ...
    fn try_with_untracked<O>(&self, f: impl FnOnce(&T) -> O) -> Option<O> {
        with_runtime(|runtime| {
            self.id
                .try_with_no_subscription(runtime, |v: &Option<T>| v.as_ref().map(f))
                .ok()
                .flatten()
        })
        .ok()
