...
use crate::{console_warn, create_effect, diagnostics, on_cleanup, spawn_local, Runtime};
...
pub mod prelude {
    ...
    pub use crate::signal_wrappers_read::*;
    pub use crate::signal_wrappers_write::*;
}
...
pub trait SignalWith {
    ...
    /// Subscribes to this signal in the current reactive scope without doing anything with its
    /// value.
    fn track(&self) {
        _ = self.try_with(|_| {});
    }
...
pub trait SignalDispose {
    ...
    /// 1. Detaches the signal from the reactive graph, preventing it from triggering further
    ///    updates; and
...
/// - [`.get()`](#impl-SignalGet<T>-for-ReadSignal<T>) (or calling the signal as a function) clones
///   the current value of the signal. If you call it within an effect, it will cause that effect to
///   subscribe to the signal, and to re-run whenever the value of the signal changes.
///   - [`.get_untracked()`](#impl-SignalGetUntracked<T>-for-ReadSignal<T>) clones the value of the
///     signal
///   without reactively tracking it.
/// - [`.with()`](#impl-SignalWith<T>-for-ReadSignal<T>) allows you to reactively access the
///   signal's value without cloning by applying a callback function.
///   - [`.with_untracked()`](#impl-SignalWithUntracked<T>-for-ReadSignal<T>) allows you to access
///     the signal's
///   value without reactively tracking it.
/// - [`.to_stream()`](#impl-SignalStream<T>-for-ReadSignal<T>) converts the signal to an `async`
///   stream of values.
///
/// # Examples
/// ```
...
impl<T: Clone> SignalGetUntracked for ReadSignal<T> {
    ...
    fn get_untracked(&self) -> T {
        match with_runtime(|runtime| self.id.try_with_no_subscription(runtime, T::clone))
    ...
    fn try_get_untracked(&self) -> Option<T> {
        with_runtime(|runtime| self.id.try_with_no_subscription(runtime, Clone::clone).ok())
            .ok()
            .flatten()
    }
}
...
impl<T> SignalWithUntracked for ReadSignal<T> {
    ...
    fn try_with_untracked<O>(&self, f: impl FnOnce(&T) -> O) -> Option<O> {
        match with_runtime(|runtime| self.id.try_with_no_subscription(runtime, f)) {
            Ok(Ok(o)) => Some(o),
            _ => None,
        }
    }
...
impl<T: Clone> SignalGet for ReadSignal<T> {
    ...
    fn get(&self) -> T {
        ...
        match with_runtime(|runtime| self.id.try_with(runtime, T::clone, diagnostics))
            .expect("runtime to be alive")
        {
            Ok(t) => t,
            Err(_) => panic_getting_dead_signal(
    ...
    pub(crate) fn with_no_subscription<U>(&self, f: impl FnOnce(&T) -> U) -> U {
        ...
        self.id
            ...
            .unwrap_or_else(|_| {
                ...
                {
                    panic!(
                        "at {}, tried to access ReadSignal<{}> defined at {}, but it has already \
                         been disposed",
                        caller,
                        std::any::type_name::<T>(),
                        self.defined_at
...
///
/// ## Core Trait Implementations
/// - [`.set()`](#impl-SignalSet<T>-for-WriteSignal<T>) (or calling the setter as a function) sets
///   the signal's value, and notifies all subscribers that the signal's value has changed. to
///   subscribe to the signal, and to re-run whenever the value of the signal changes.
...
///   - [`.update_untracked()`](#impl-SignalUpdateUntracked<T>-for-WriteSignal<T>) mutates the
///     signal's value
...
///
/// ## Core Trait Implementations
/// - [`.get()`](#impl-SignalGet<T>-for-RwSignal<T>) clones the current value of the signal. If you
///   call it within an effect, it will cause that effect to subscribe to the signal, and to re-run
///   whenever the value of the signal changes.
///   - [`.get_untracked()`](#impl-SignalGetUntracked<T>-for-RwSignal<T>) clones the value of the
///     signal
///   without reactively tracking it.
/// - [`.with()`](#impl-SignalWith<T>-for-RwSignal<T>) allows you to reactively access the signal's
///   value without cloning by applying a callback function.
///   - [`.with_untracked()`](#impl-SignalWithUntracked<T>-for-RwSignal<T>) allows you to access the
///     signal's
///   value without reactively tracking it.
/// - [`.set()`](#impl-SignalSet<T>-for-RwSignal<T>) sets the signal's value, and notifies all
///   subscribers that the signal's value has changed. to subscribe to the signal, and to re-run
///   whenever the value of the signal changes.
///   - [`.set_untracked()`](#impl-SignalSetUntracked<T>-for-RwSignal<T>) sets the signal's value
///   without notifying its subscribers.
/// - [`.update()`](#impl-SignalUpdate<T>-for-RwSignal<T>) mutates the signal's value in place and
///   notifies all subscribers that the signal's value has changed.
///   - [`.update_untracked()`](#impl-SignalUpdateUntracked<T>-for-RwSignal<T>) mutates the signal's
///     value
///   in place without notifying its subscribers.
/// - [`.to_stream()`](#impl-SignalStream<T>-for-RwSignal<T>) converts the signal to an `async`
///   stream of values.
///
/// ```
/// # use leptos_reactive::*;
    ...
    fn get_untracked(&self) -> T {
        ...
        self.id
            ...
            .unwrap_or_else(|_| {
                ...
                {
                    panic!(
                        "at {}, tried to access RwSignal<{}> defined at {}, but it has already \
                         been disposed",
                        caller,
                        std::any::type_name::<T>(),
                        self.defined_at
    ...
    fn with_untracked<O>(&self, f: impl FnOnce(&T) -> O) -> O {
        self.id
            ...
            .unwrap_or_else(|_| {
                ...
                {
                    panic!(
                        "tried to access RwSignal<{}> defined at {}, but it has already been \
                         disposed",
                        std::any::type_name::<T>(),
                        self.defined_at
                    )
...
impl<T: Clone> SignalGet for RwSignal<T> {
    ...
    fn try_get(&self) -> Option<T> {
        ...
        with_runtime(|runtime| self.id.try_with(runtime, Clone::clone, diagnostics).ok())
            .ok()
            .flatten()
    }
}
...
impl NodeId {
            ...
            #[cfg(all(debug_assertions, not(feature = "ssr")))]
            {
                if !SpecialNonReactiveZone::is_inside() {
                    ...
                    crate::macros::debug_warn!(
                        "At {called_at}, you access a signal or memo (defined at {defined_at}) \
                         ...
                         how to fix it:\n\n1. If this is inside a `view!` macro, make sure you \
                         are passing a function, not a value.\n  ❌ NO  <p>{{x.get() * 2}}</p>\n  \
                         ✅ YES <p>{{move || x.get() * 2}}</p>\n\n2. If it's in the body of a \
                         component, try wrapping this access in a closure: \n  ❌ NO  let y = \
                         x.get() * 2\n  ✅ YES let y = move || x.get() * 2.\n\n3. If you're \
                         *trying* to access the value without tracking, use `.get_untracked()` or \
    ...
    fn update_value<T, U>(
        ...
        #[cfg(debug_assertions)] defined_at: Option<&'static std::panic::Location<'static>>,
    ) -> Option<U>
    where
        T: 'static,
    {
            ...
            if let Some(value) = runtime.get_value(*self) {
                ...
                #[cfg(debug_assertions)]
                {
                    if let Some(defined_at) = defined_at {
                        debug_warn!(
                            "[Signal::update] At {:?}, you're trying to update a Signal<{}> \
                             (defined at {defined_at}) that has already been disposed of. This is \
                             probably a logic error in a component that creates and disposes of \
                             scopes. If it does not cause any issues, it is safe to ignore this \
