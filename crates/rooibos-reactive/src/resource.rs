...
/// # // because we're not running in a browser or in Tokio. Let's just ignore it.
/// # if false {
/// let result = create_local_resource(move || (), |_| setup_complicated_struct());
/// # }
/// # runtime.dispose();
/// ```
#[cfg_attr(
...
{
    #[track_caller]
    pub fn read(&self, location: &'static Location<'static>, id: ResourceId) -> Option<T>
    where
        T: Clone,
    {
    ...
    pub fn with<U>(
        ...
        self.handle_result(location, global_suspense_cx, suspense_cx, v, false, id)
    }
    ...
    fn load(&self, refetching: bool, id: ResourceId) {
        ...
        _ = self.source.try_with_untracked(|source| {
            ...
            for suspense_context in suspense_contexts.borrow().iter() {
                suspense_context
                    .increment_for_resource(self.serializable != ResourceSerialization::Local, id);
    ...
    fn dispose(self) {
        ...
        if res.ok().flatten().is_none() {
            crate::macros::debug_warn!(
                "At {}, you are calling Resource::dispose() on a resource that no longer exists, \
                 probably because its Scope has already been disposed.",
