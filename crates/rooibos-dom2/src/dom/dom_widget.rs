...
// margin_right, margin_top,     margin_x, margin_y, max_height, max_width, min_height,
// min_width, padding, padding_bottom,     padding_left, padding_right, padding_top, padding_x,
// padding_y, position, shrink, width,     AlignSelf, AspectRatio, Basis, Border, BorderBottom,
// BorderLeft, BorderRight, BorderTop,     BorderX, BorderY, Clear, Disabled, Focusable, Grow,
// Height, Margin, MarginBottom, MarginLeft,     MarginRight, MarginTop, MarginX, MarginY,
...
impl DomWidgetNode {
    ...
    // pub fn update<F1, F2>(&mut self, f: F1)
    // where
    //     F1: Fn() -> F2 + 'static,
    //     F2: FnMut(Rect, &mut Buffer) + 'static,
    // {
    //     self.f = move || Box::new((f)());
    //     self.build();
    // }
}
